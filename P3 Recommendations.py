...
movieNames = np.loadtxt("./ml-100k/u.item", delimiter = "|", usecols = (0, 1), dtype = {'names': ('id', 'name'),
                                                                                        'formats': (np.int, 'S128')})
...
movieData = np.loadtxt("./ml-100k/u.data", delimiter = "\t", usecols = (0, 1, 2), dtype = {'names': ('userID', 'movieID', 'rating'),
...
for movie in movieData:
    if movie['movieID'] in movieRatingTemp:
        movieRatingTemp[movie['movieID']].append(movie['rating'])
    else:
        movieRatingTemp[movie['movieID']] = [movie['rating']]
...
for movie in movieRatingTemp:
    movieRating[movie] = np.mean(movieRatingTemp[movie])
...
# Get sorting ratings
# https://www.saltycrane.com/blog/2007/09/how-to-sort-python-dictionary-by-keys/b j
